...
export function enemyGainPower(e, amount) {
  ...
  if (nr && nr !== e.rank) {
    e.rank = nr; e.rankLabel = nr; applyEnemyRankBenefits(e);
    enforceILvForRank(e);
  }
}
...
export function updateEnemies(dt, safeZones) {
  for (const e of enemies) {
    ...
    if (target) {
      ...
      // timers init
      e._time = (e._time || 0) + dt;
      if (e._phase === undefined) e._phase = Math.random() * Math.PI * 2;        // desync
      e._strafeFlipT = (e._strafeFlipT || (1.2 + Math.random() * 1.2)) - dt;   // next flip in 1.2–2.4s
      if (e._strafeFlipT <= 0) {
        if (Math.random() < 0.5) e._strafeDir = (e._strafeDir === 1 ? -1 : 1);
        e._strafeFlipT = 1.2 + Math.random() * 1.8;
      }
      const baseDir = (e._strafeDir ?? (Math.random() < 0.5 ? 1 : -1));
      const osc = Math.sign(Math.sin(e._time * 1.7 + e._phase)) || 1;
      const dir = baseDir * osc;

      // Stop-and-shoot holds to break orbit sync when in good range
      e._holdT = Math.max(0, (e._holdT || 0) - dt);
      const inBand = !tooFar && !tooClose;
      if (inBand && d < desired + 120 && e._holdT === 0 && Math.random() < 0.06) {
        e._holdT = 0.35 + Math.random() * 0.4;
      }
      ...
      if (e._holdT > 0) {
        ...
        const radialBias = 0.18 * Math.sign(desired - d);
        mvx = sx * dir * (1 - Math.abs(radialBias)) + (dxT / d) * radialBias;
        mvy = sy * dir * (1 - Math.abs(radialBias)) + (dyT / d) * radialBias;
      }

      // Soft center bias and edge steering to avoid drifting off map
      const centerX = (mapW || 4000) * 0.5, centerY = (mapH || 4000) * 0.5;
      const edgeMargin = 96;
      if (e.x < edgeMargin) mvx += 0.4;
      if (e.x > (mapW - edgeMargin)) mvx -= 0.4;
      if (e.y < edgeMargin) mvy += 0.4;
      if (e.y > (mapH - edgeMargin)) mvy -= 0.4;
      const toCenterX = centerX - e.x, toCenterY = centerY - e.y;
      const dc = Math.hypot(toCenterX, toCenterY);
      if (dc > Math.max(mapW, mapH) * 0.35) {
        mvx += (toCenterX / (dc || 1)) * 0.25;
        mvy += (toCenterY / (dc || 1)) * 0.25;
      }
