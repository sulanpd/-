...
function handleSkillKey(k){
  ...
  if (player.advancedClass === "Berserker") {
    ...
    if (k===2){
      ...
      for (let i=0;i<enemies.length;i++){ const e=enemies[i]; if(!e.alive) continue;
  ...
  for (const pb of playerBullets) {
    ...
    for (const e of enemies) {
      ...
      if (hit) {
        ...
        if (player.advancedClass==='Berserker'){ gainClassBar(Math.min(6, dmg*0.02)); }
        ...
        if (player.classBuffs?.rageTankStacks){
          ...
          const idxT = enemies.indexOf(e);
          const same = (st.lastTargetId === idxT);
          if (same) st.stacks = Math.min(3000, st.stacks + 1); else st.stacks = 1;
          st.lastTargetId = idxT;
        }
        if (e.hp <= 0) {
          e.alive = false;
...
function maybeShowClassPanel(){
  ...
  if (can){
    const isDPS = player.rebornClass === "DPS";
    const b = document.getElementById('pickBerserker');
    const l = document.getElementById('pickLadino');
    const r = document.getElementById('pickRageTank');
    const p = document.getElementById('pickPaladino');
    if (b) b.style.display = isDPS? 'block':'none';
    if (l) l.style.display = isDPS? 'block':'none';
    if (r) r.style.display = !isDPS? 'block':'none';
    if (p) p.style.display = !isDPS? 'block':'none';
  }
}
