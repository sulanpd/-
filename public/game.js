...
function update() {
    ...
    if (!player.alive) {
        ...
        if (player.respawnTimer <= 0) {
            ...
            if (player.level > 25) player.deathsAfter25++;
            // "Vontade dos fracos"
            if (!player.conquest.vontadeFracos && player.level > 10 && player.deathsAfter10 == 1) {
                player.conquest.vontadeFracos = true;
                showConquestMsg('Conquista: "Vontade dos fracos"! Dano +10%, XP +25%, Redução de dano +5%!');
            }
            // "Nunca Desistir"
            if (!player.conquest.nuncaDesistir && player.level > 25 && player.deathsAfter25 == 5) {
                player.conquest.nuncaDesistir = true;
                player.points += 10;
                showConquestMsg('Conquista: "Nunca Desistir"! Alcance do tiro DOBRADO, +10 pontos!');
            }
    ...
    // -- (continua shooters, boss, conquista, etc no próximo bloco) --
}
// game.js — Bloco 4B
